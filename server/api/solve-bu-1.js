export default defineEventHandler(async (event) => {
	...
	const mainLoop = async (index, board, piecesRemaining, state = []) => {
		...
		const elFin = index > pieces.length;
		if (elFin) return state;
		...
		const deadEnd = possiblePieces.length === 0 && piecesRemaining.length >= 0;

		if (deadEnd) {
			// If there are no possible pieces, we need to backtrack:
			const lastBranch = branchExploration.pop();
			state = state.slice(0, lastBranch);
			// return state;
		}
		...
		if (possiblePieces.length === 1) {
			...
			state = await mainLoop(index + 1, board, piecesRemaining, state);
		} else {
	...
	const solution = await mainLoop(0, board, pieces);
