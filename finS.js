...
const boardClues = [
	...
	{ left: '*', right: '*', bottom: '*' },
	{ left: '*', right: 'green', bottom: 'orange' },
...
const main = (index = 0, state = [], piecesRemaining = pieces) => {
	...
	possibilities.push(possiblePieces.length);

	// Handle dead end:
	if (possiblePieces.length === 0 && piecesRemaining.length >= 0) {
		console.log('Dead end');
		if (state.length > bestRun.length) {
			bestRun = state;
		}
	}
...
// console.log('The best run:', bestRun);
// console.log('with length of: ', bestRun.length);
